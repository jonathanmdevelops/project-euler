...
def largest_horizontal(grid):
	largest = 0
	for row in grid:
		for column in range(0, len(row) - 3):
			product = row[column] * row[column + 1] * row[column + 2] * row[column + 3]
...
def largest_vertical(grid):
	...
	for column in range(0, len(grid) - 3):
		for row in range(0, len(grid)):
			product = grid[column][row] * grid[column + 1][row] * grid[column + 2][row] * grid[column + 3][row]
			largest = max(product, largest)
	return largest

def largest_left_diagonal(grid):
	largest = 0
	for column in range(0, len(grid) - 3):
		for row in range(0, len(grid) - 3):
			product = grid[column][row] * grid[column + 1][row + 1] * grid[column + 2][row + 2] * grid[column + 3][row + 3]
			largest = max(product, largest)
	return largest

def largest_right_diagonal(grid):
	largest = 0
	for column in range(len(grid) - 1, 2, -1):
		for row in range(0, len(grid) - 3):
			product = grid[column][row] * grid[column - 1][row + 1] * grid[column - 2][row + 2] * grid[column - 3][row + 3]
			largest = max(product, largest)
	return largest

print max(largest_horizontal(grid), largest_vertical(grid), largest_left_diagonal(grid), largest_right_diagonal(grid))